...
export default class GLightbox {
	...
	constructor(options: Partial<GLightboxOptions> = {}) {
		...
		if (
			...
			this.options.appearance?.slideEffect
		) {
	...
	public goToSlide(index = 0): void {
		...
		if (
			!this.options.loop &&
			(index < 0 || index > this.getTotalSlides() - 1)
		) {
			return;
		}
		if (index < 0) {
	...
	private async showSlide(index = 0, first = false): Promise<void> {
		const current =
			this.slidesContainer?.querySelector<HTMLElement>(".current");
		if (current) {
			removeClass(current, "current");
		}

		const slideNode = this.slidesContainer?.querySelectorAll(".gl-slide")[
			index
		] as HTMLElement;
		...
		if (!first) {
			this.trigger("slide_before_change", {
				current: this.state.get("prevActiveSlideIndex"),
				next: index,
			});
		}

		const effect = this.reduceMotion
			? false
			: this.options.appearance?.slideEffect;
		const openEffect = this.reduceMotion
			? false
			: this.options.appearance?.openEffect;
		...
		this.trigger("slide_changed", {
			prev: this.state.get("prevActiveSlideIndex"),
			current: index,
		});
	...
	private async preloadSlide(
		...
		if (
			slideNode &&
			(hasClass(slideNode, "loaded") || hasClass(slideNode, "preloading"))
		) {
			return true;
		}
	...
	private build(): void {
		...
		addClass(
			this.modal,
			`gl-theme-${this.options?.appearance?.theme ?? "base"}`,
		);
		addClass(
			this.modal,
			`gl-slide-effect-${this.options?.appearance?.slideEffect || "none"}`,
		);

		if (this.options?.appearance?.cssVariables) {
			for (const [key, value] of Object.entries(
				this.options.appearance.cssVariables,
			)) {
			...
			addEvent("click", {
				...
				callback: (target: HTMLElement, e: Event) => {
					if (
						target &&
						e?.target &&
						!(e?.target as HTMLElement)?.closest(".gl-media")
					) {
			...
			(entries) => {
				for (const entry of entries) {
					...
					if (entry.isIntersecting && this.state.get("open")) {
						const enteredIndex = Number.parseInt(
						...
						if (
							!hasClass(entry.target, "loaded") &&
							!hasClass(entry.target, "preloading")
						) {
		...
		for (const item of this.items) {
			...
			if (item?.url && slideType) {
				...
				const created =
					this.slidesContainer?.querySelectorAll(".gl-slide")[index];
				if (created) {
	...
	public async close(): Promise<void> {
		...
		if (this.reduceMotion || this.options.appearance?.slideEffect === "none") {
			...
			const media = (currentSlide as HTMLElement).querySelector<HTMLElement>(
				".gl-media",
			);
	...
	public setItems(items: SlideConfig[]): void {
		...
		for (const item of items) {
			...
			for (const [key, slideType] of slideModules) {
				...
				if (
					slideType?.options &&
					typeof slideType.options?.matchFn === "function"
				) {
	...
	private setItemsFromNode(node: HTMLElement): void {
		...
		for (const item of items) {
			const itemData: SlideConfig = this.parseConfigFromNode(
				item as HTMLElement,
			);
	...
	private parseConfigFromNode(element: HTMLElement): SlideConfig {
		...
		for (const key in slideDefaults) {
			...
			let nodeData: string | boolean | null = element.getAttribute(
				`${attr}-${key}`,
			);
			if (nodeData) {
	...
	public async injectAssets(
		urls: (string | string[] | { src: string; module?: boolean })[],
	): Promise<void> {
