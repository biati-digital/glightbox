...
export default class KeyboardNavigation extends GLightboxPlugin {
	...
	init(): void {
		const focusableElements =
			this.instance.modal?.querySelectorAll("*[tabindex]");
		if (focusableElements) {
			const focusablIndexes = [...focusableElements].map((el) =>
				Number.parseInt(el.getAttribute("tabindex") ?? "0"),
			);
	...
	onKeyDown(event: KeyboardEvent) {
		...
		switch (key) {
			case "Tab":
				if (
					this.largest > 0 &&
					instance.modal?.contains(document.activeElement)
				) {
					const activeFocusIndex =
						document.activeElement?.getAttribute("tabindex") ?? "0";
					let nextFocusIndex = shiftKey
						? Number.parseInt(activeFocusIndex) - 1
						: Number.parseInt(activeFocusIndex) + 1;
					...
					const toFocus = instance.modal?.querySelector(
						`*[tabindex="${nextFocusIndex}"]`,
					) as HTMLElement;
					if (toFocus) {
						toFocus.focus();
					}
